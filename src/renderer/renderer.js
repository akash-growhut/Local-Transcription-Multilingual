...
function setupEventListeners() {
  ...
  window.electronAPI.onMicrophoneConnected((connected) => {
    updateStatus('micStatus', connected ? 'Recording' : 'Ready', connected ? 'recording' : '')
  })

  window.electronAPI.onSpeakerConnected((connected) => {
    updateStatus('speakerStatus', connected ? 'Recording' : 'Ready', connected ? 'recording' : '')
  })
      ...
      setTimeout(() => {
        if (
          ...
          updateStatus(
            ...
            speakerStatus.classList.contains('recording') ? 'recording' : 'connected'
          )
        }
      }, 5000)
    }
...
async function startAll() {
  ...
  try {
    ...
    if (isElectron) {
      ...
      const audioResult = await audioCapture.startSpeakerCapture((audioData, source) => {
    ...
    setTimeout(() => {
      ...
      const hasTranscripts =
        chatMessages && chatMessages.querySelectorAll('.speaker-message').length > 0
...
function displaySequentialTranscripts() {
  ...
  while (speakerTranscripts.size > 0) {
    ...
    const sortedIndices = Array.from(speakerTranscripts.keys()).sort((a, b) => a - b)
    ...
    const adjustedText = adjustCapitalization(transcriptData.text, lastDisplayedText)
...
function displayTranscript(text, isFinal, source, eventData = null) {
  ...
  console.log(`📝 Received live ${source} transcript (${isFinal ? 'FINAL' : 'interim'}): "${text}"`)
  ...
  if (isFinal) {
    ...
    const messageDiv = document.createElement('div')
    messageDiv.className = `message ${source === 'microphone' ? 'user-message' : 'speaker-message'}`
...
function updatePlatformInfo() {
  ...
  if (audioCapture && typeof audioCapture.checkSystemAudioSupport === 'function') {
  ...
  const updateConnectionStatus = () => {
    const micRecording = document.getElementById('micStatus').classList.contains('recording')
