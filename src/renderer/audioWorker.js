  ...
  process(samples) {
    ...
    for (let i = 0; i < samples.length; i++) {
      ...
      if (inputLevel > this.envelope) {
        this.envelope = attackCoef * this.envelope + (1.0 - attackCoef) * inputLevel
        this.holdCounter = this.holdTime * this.sampleRate
      } else {
        if (this.holdCounter > 0) {
          this.holdCounter--
        } else {
          this.envelope = releaseCoef * this.envelope + (1.0 - releaseCoef) * inputLevel
        }
      }
...
class SpectralNoiseReduction {
  ...
  updateNoiseProfile(samples) {
    for (let i = 0; i < Math.min(samples.length, this.frameSize); i++) {
      ...
      this.noiseProfile[i] =
        this.noiseProfile[i] * this.smoothingFactor + absVal * (1.0 - this.smoothingFactor)
    }
  }
  ...
  process(samples) {
    ...
    for (let i = 0; i < samples.length; i++) {
      ...
      const noise = i < this.frameSize ? this.noiseProfile[i] : this.noiseProfile[0]
...
self.onmessage = function (e) {
  ...
  switch (type) {
    ...
    case 'set-enabled':
      ...
      console.log(`🎤 Worker: Noise cancellation ${processingEnabled ? 'enabled' : 'disabled'}`)
      self.postMessage({
